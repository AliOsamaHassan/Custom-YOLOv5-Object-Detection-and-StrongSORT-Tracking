...
def run(
    ...
    inference_images_dir = increment_path(
        Path(project) / "inference_images", exist_ok=exist_ok
    )
    (inference_images_dir).mkdir(parents=True, exist_ok=True)  # make dir
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if save_txt:
                            ...
                            with open(txt_path + "_detections.txt", "a") as f:
                                f.write(
                                    ("%g " * 11 + "\n")
                                    % (
                                        frame_idx + 1,
            ...
            if count:
                ...
                try:
                    df = pd.read_csv(
                        txt_path + "_detections.txt", header=None, delim_whitespace=True
                    )
                    df = df.iloc[:, 0:3]
                    ...
                    for key, val in enumerate(names):
                        vc2[key] = val
                    itemDict = dict(
                        (names[vc2[key]], value) for (key, value) in vc.items()
                    )
            ...
            if save_img and det is not None and len(det):
                out_img_path = os.path.join(
                    str(inference_images_dir), "{}.jpg".format(frame_idx)
                )
                print("frame_idx:", out_img_path)
            ...
            if save_vid:
                if vid_path[i] != save_path:  # new video
                    ...
                    save_path = save_path.replace(".mp4", "_inference_video.mp4")
    ...
    if save_txt or save_vid:
        s = (
            f"\n{len(list(save_dir.glob('*.txt')))} tracks saved to {save_dir}"
            if save_txt
            else ""
        )
